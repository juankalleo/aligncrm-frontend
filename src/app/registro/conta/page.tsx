...
export default function RegistroContaPage() {
  ...
  const onSubmit = async (data: RegistroForm) => {
    try {
      ...
      // If user chose to join a workspace, send request and redirect to login
      if (flow === 'join' && (joinDraft.projetoId || joinDraft.codigo)) {
        ...
        // Redirect to login after join request
        toast.info('Faça login para acessar o sistema.');
        router.push('/login');
        return;
      }

      // after registration, perform workspace action if chosen
      try {
        if (flow === 'create' && createDraft.nome && createDraft.aceitarTermos) {
          ...
          if (novo?.id) {
            // persist workspace if backend returned it
      ...
      // If email already exists, redirect user to login with draft preserved
